...
public class AuthenticatorTest extends BaseTest {
    ...
    public void testBasicAuthenticatorInstance() {
        ...
        BasicAuthenticator auth = new BasicAuthenticator(username, password.getBytes(Charset.defaultCharset()));
        assertEquals(username, auth.getUsername());
        assertEquals(password, new String(auth.getPasswordBytes(), Charset.defaultCharset()));
    }

    @SuppressWarnings("ConstantConditions")
    @Test(expected = IllegalArgumentException.class)
    public void testBasicAuthenticatorWithEmptyUsername() {
        new BasicAuthenticator(null, "somePassword".getBytes(Charset.defaultCharset()));
    }

    @SuppressWarnings("ConstantConditions")
    @Test(expected = IllegalArgumentException.class)
    public void testBasicAuthenticatorWithEmptyPassword() { new BasicAuthenticator("someUsername", (byte[]) null); }
