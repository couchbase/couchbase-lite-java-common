...
class C4QueryObserverTest : C4BaseTest() {
    ...
    private val callbackMock = C4QueryObserver.QueryChangeCallback() { _, _, _ ->
        callCount++
    }
    ...
    fun testCreateC4QueryObserver() {
        ...
        val token = ChangeListenerToken<QueryChange>(null, ChangeListener { _ -> })
        assertEquals(0, C4QueryObserver.QUERY_OBSERVER_CONTEXT.size())
        val observer = C4QueryObserver.create(c4Query, token, callbackMock)
        assertNotNull(observer)
        assertEquals(1, C4QueryObserver.QUERY_OBSERVER_CONTEXT.size())

        //create a second observer should increase size
        val token2 = ChangeListenerToken<QueryChange>(null, ChangeListener { _ -> })
        val observer2 = C4QueryObserver.create(c4Query, token2, callbackMock)
    ...
    fun testCloseC4QueryObserver() {
        ...
        val token = ChangeListenerToken<QueryChange>(null, ChangeListener { _ -> })
        val observerClose = C4QueryObserver.create(c4Query2, token, callbackMock)
        observerClose.close()
        assertEquals(0, C4QueryObserver.QUERY_OBSERVER_CONTEXT.size())
    }
    ...
    fun testQueryChanged() {
        ...
        val token = ChangeListenerToken<QueryChange>(null, ChangeListener { _ -> Unit  })
        val c4QueryObserver = C4QueryObserver(0xdacL, c4QueryObserverMock, c4Query, token, callbackMock)
