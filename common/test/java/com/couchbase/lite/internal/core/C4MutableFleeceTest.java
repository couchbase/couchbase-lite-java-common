...
public class C4MutableFleeceTest extends C4BaseTest {
    ...
    public void testMValue() {
        ...
        Assert.assertEquals("hi", val.asNative(null));
        Assert.assertNull(val.getValue());
    }
    ...
    public void testMRoot2() throws LiteCoreException {
        ...
        FLSliceResult encodedRoot1 = encode(root);
        Assert.assertEquals("{greeting:\"hi\"}", fleece2JSON(encodedRoot1));

        FLSliceResult encodedRoot2 = encodeRoot(root);
        Assert.assertEquals("{greeting:\"hi\"}", fleece2JSON(encodedRoot2));
        ...
        Assert.assertEquals(expected, fleece2JSON(encodeObj(root.asNative())));
        Assert.assertEquals(expected, fleece2JSON(encode(root)));
        Assert.assertEquals(expected, fleece2JSON(encodeRoot(root)));
    }
    ...
    @NonNull
    private FLSliceResult encode(MCollection collection) throws LiteCoreException {
        try (FLEncoder encoder = FLEncoder.getManagedEncoder()) {
            collection.encodeTo(encoder);
