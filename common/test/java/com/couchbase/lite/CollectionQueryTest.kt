...
class CollectionQueryTest : BaseQueryTest() {
    ...
    fun testQueryDefaultCollectionA() {
        ...
        verifyQuery(
            ...
            Assert.assertEquals(1, result.count().toLong())
            Assert.assertEquals("{\"first\":\"Abe\"}", result.toJSON())
            Assert.assertEquals("Abe", result.getValue("first"))
            Assert.assertEquals(result.getValue(0), result.getValue("first"))
        }
    }
    ...
    fun testSQLPPJoinWithCollectionsA() {
        ...
        doc.setValue("cid", "c1")
        saveDocumentInCollection(doc, flowerCol)
        doc = MutableDocument()
        ...
        saveDocumentInCollection(doc, colorCol)
        doc = MutableDocument()
        doc.setValue("cid", "c2")
        ...
        verifyQuery(query, 2) { _, result ->
            when (val name = result.getString("name")) {
                "hydrangea" -> Assert.assertEquals("blue", result.getString("color"))
                "rose" -> Assert.assertEquals("red", result.getString("color"))
                else -> Assert.fail("unexpected name: ${name}")
            }
            Assert.assertEquals(2, result.count().toLong())
            Assert.assertEquals(result.getValue(0), result.getValue("name"))
            Assert.assertEquals(result.getValue(1), result.getValue("color"))
        }
    }
    ...
    fun testFTSJoinQuery() {
        ...
        verifyQuery(query, 1) { _, result ->
            Assert.assertEquals(3, result.count().toLong())
            Assert.assertEquals("rose", result.getValue("name"))
            Assert.assertEquals("Red flowers", result.getValue("description"))
            Assert.assertEquals("red", result.getValue("color"))
        }
    }
    ...
    fun testSelectAllResultKeyA() {
        ...
        doc.setValue("description", "Red flowers")
        doc.setValue("cid", "c1")
        saveDocumentInCollection(doc, defaultCollection)
        doc = MutableDocument()
        doc.setValue("name", "hydrangea")
        doc.setValue("description", "Blue flowers")
        doc.setValue("cid", "c2")
    ...
    fun testSelectAllResultKeyB() {
        ...
        query.execute().use { rs ->
            val colNames = rs.columnNames
            Assert.assertEquals(1, colNames.size)
            Assert.assertEquals("_", colNames[0])
        }
    }
    ...
    fun testSelectAllResultKeyC() {
        ...
        query.execute().use { rs ->
            val colNames = rs.columnNames
            Assert.assertEquals(1, colNames.size)
            Assert.assertEquals(defaultCollection.name, colNames[0])
        }
    }
    ...
    fun testSelectAllResultKeyD() {
        ...
        doc.setValue("description", "Red flowers")
        doc.setValue("cid", "c1")
        saveDocumentInCollection(doc, testCollection)
        doc = MutableDocument()
        doc.setValue("name", "hydrangea")
        doc.setValue("description", "Blue flowers")
        doc.setValue("cid", "c2")
        ...
        query.execute().use { rs ->
            val colNames = rs.columnNames
            Assert.assertEquals(1, colNames.size)
            Assert.assertEquals(testCollection.name, colNames[0])
        }
    }
    ...
    fun testSelectAllResultKeyE() {
        ...
        query.execute().use { rs ->
            val colNames = rs.columnNames
            Assert.assertEquals(1, colNames.size)
            Assert.assertEquals("f", colNames[0])
        }
    }
    ...
    fun testBuilderSelectAllResultKeyB() {
        ...
        query.execute().use { rs ->
            val colNames = rs.columnNames
            Assert.assertEquals(1, colNames.size)
            Assert.assertEquals("foo", colNames[0])
        }
    }
