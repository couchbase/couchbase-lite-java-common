...
public final class FLDict extends FLContainerSlice<FLDict.NativeImpl> {
    ...
    public FLValue get(@Nullable String key) {
        return (key == null) ? null : childAt(peer -> impl.nGet(peer, key.getBytes(StandardCharsets.UTF_8)));
    }
    ...
    public Map<String, Object> asDict() {
        ...
        try (FLDictIterator itr = iterator()) {
            ...
            while ((key = itr.getKey()) != null) {
                final FLValue val = itr.getFLValue();
                results.put(key, val.toJava());
                itr.next();
            }
        }
        return results;
